...
int main(int argc, char* argv[])
{
    ...
    while (1)
    {
         ...
         */
#ifdef __ANDROID__
        /* New characters have no save to load. play_game() chooses their
         * savefile after character creation, as on Windows. */
        if (new_game)
            savefile[0] = '\0';
#endif
        play_game(new_game);
