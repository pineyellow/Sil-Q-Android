 ...
 *
 * On desktop, this function allows the user to save (or quit) the game
 * when he dies, since the "You die." message is shown before setting
 * the player to "dead". Android records death before that message so
 * stopping the app at the prompt cannot restore the last living save.
 */
void take_hit(int dam, cptr kb_str)
{
    ...
    if (dam > 0) android_interrupt_dpad();
#endif
    p_ptr->chp -= dam;
    ...
    {
#if defined(__ANDROID__) && defined(USE_SDL)
        /* Retire before either message call can block or the app can suspend. */
    ...
    for (i = 0; i < INVEN_PACK; i++)
    {
        ...
        if ((*typ)(o_ptr))
        {
            ...
            if (amt)
            {
                ...
                /* Message */
                msg_format("%sour %s%s %s destroyed!",
                    ((o_ptr->number > 1)
                            ? ((amt == o_ptr->number)
                                      ? "All of y"
                                      : (amt > 1 ? "Some of y" : "One of y"))
                            : "Y"),
                    o_name, item_message_label(i), ((amt > 1) ? "were" : "was"));
...
bool apply_disenchant(int mode)
{
    ...
    {
        /* Message */
        msg_format("Your %s%s resist%s disenchantment!", o_name,
            item_message_label(t), ((o_ptr->number != 1) ? "" : "s"));
    ...
    msg_format("Your %s%s %s disenchanted!", o_name, item_message_label(t),
