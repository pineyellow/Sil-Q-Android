 ...
 * Header for additional information when printing to screen.
 */
static bool screen_out_head(const object_type* o_ptr, bool show_name)
{
    char* o_name;
    ...
    /* Print, in colour */
    if (show_name) text_out_c(TERM_YELLOW, format("%^s", o_name));
...

    {
        ...
        /* Keep the original lore, replacing only its desktop control hint. */
        cptr lore = a_text + a_info[o_ptr->name1].text;
        cptr keyboard_hint = o_ptr->name1 == ART_MORGOTH_3
            ? strstr(lore, "stand over it and press (k).") : NULL;
        if (keyboard_hint)
        {
            p_text_out(format("%.*s", (int)(keyboard_hint - lore), lore));
            p_text_out("stand over it, tap its picture and choose Destroy.");
            p_text_out(keyboard_hint + strlen("stand over it and press (k)."));
        }
        else
#endif
...
{
#if defined(__ANDROID__) && defined(USE_SDL)
    extern void android_show_note(cptr text);
#if defined(SILQ_DEBUG_TUTORIAL_SKILLS)
    extern bool android_debug_tutorial_enabled(void);
    if (p_ptr->game_type < 0 && o_ptr->tval == TV_NOTE && o_ptr->sval == 1
        && android_debug_tutorial_enabled())
    {
        android_show_note("Android debug mode on. If you're seeing this on a release "
            "version please report it to me (github.com/pineyellow)");
        return;
    }
#endif
    android_show_note(k_text + k_info[o_ptr->k_idx].text);
    return;
#endif
...
void object_info_screen(const object_type* o_ptr)
{
    ...
    has_description = screen_out_head(o_ptr, TRUE);
...
#if defined(__ANDROID__) && defined(USE_SDL)
/* Translate only the combat values exposed by object_desc(). */
static void android_stat_bonus(cptr label, int value)
{
    if (!value) return;
    text_out(label);
    text_out_c(value < 0 ? TERM_L_RED : TERM_L_GREEN, format("%+d", value));
    text_out("\n");
}

static void android_stat_range(cptr label, int dice, int sides, cptr suffix)
{
    if (dice <= 0 || sides <= 0) return;
    text_out(label);
    if (sides == 1)
        text_out_c(TERM_L_GREEN, format("%d", dice));
    else
        text_out_c(TERM_L_GREEN, format("%d-%d", dice, dice * sides));
    text_out(suffix);
    text_out("\n");
}

static void android_object_stats(const object_type* o_ptr)
{
    /* Unidentified rings conceal these values in the header as well. */
    if (o_ptr->tval == TV_RING && !object_known_p(o_ptr)) return;
    android_stat_bonus("Accuracy: ", o_ptr->att);
    switch (o_ptr->tval)
    {
    case TV_HAFTED:
    case TV_POLEARM:
    ...
    case TV_DIGGING:
    case TV_BOW:
        android_stat_range("Damage: ", o_ptr->dd,
            o_ptr->ds + hand_and_a_half_bonus(o_ptr), "");
        break;
    }
    android_stat_bonus("Evasion: ", o_ptr->evn);
    android_stat_range("Absorbs ", o_ptr->pd, o_ptr->ps, " damage");
}

void android_object_stat_description(const object_type* o_ptr,
    void (*sink)(byte, cptr))
{
    void (*saved_hook)(byte, cptr) = text_out_hook;
    text_out_hook = sink;
    android_object_stats(o_ptr);
    /* Weight is public even for unidentified items. Show per-item weight for
     * weapon comparisons, and the stack total when there is more than one. */
    text_out("Weight: ");
    text_out_c(TERM_L_WHITE, format("%d.%d lb", o_ptr->weight / 10, o_ptr->weight % 10));
    if (o_ptr->number > 1) {
        int total = o_ptr->weight * o_ptr->number;
        text_out_c(TERM_L_WHITE, format(" (%d.%d lb total)", total / 10, total % 10));
    }
    text_out("\n");
    text_out_hook = saved_hook;
}

/* Export the same known information without saving/drawing a terminal modal. */
void android_object_description(const object_type* o_ptr,
    void (*sink)(byte, cptr))
{
    void (*saved_hook)(byte, cptr) = text_out_hook;
    void (*saved_flags)(const object_type*, u32b*, u32b*, u32b*) = object_info_out_flags;
    bool saved_paragraph = new_paragraph;
    text_out_hook = sink;
    bool description = screen_out_head(o_ptr, FALSE);
    object_info_out_flags = object_flags_known;
    new_paragraph = TRUE;
    bool info = object_info_out(o_ptr);
    if (!object_known_p(o_ptr))
        p_text_out("\n\nThis item has not been identified.");
    else if (!description && !info)
        p_text_out("\n\nThis item does not seem to possess any special abilities.");
    text_out_hook = saved_hook;
    object_info_out_flags = saved_flags;
    new_paragraph = saved_paragraph;
}
#endif
