...
void do_cmd_inven(void)
{
    ...
    extern void android_inventory(void);
    android_inventory();
    return;
#endif
...
void do_cmd_wield(object_type* default_o_ptr, int default_item)
{
        ...
        && (inventory[INVEN_ARM].k_idx))
    {
        ...
        if ((item < 0) && (&inventory[INVEN_PACK - 1])->tval)
        {
            ...
            {
#if defined(__ANDROID__) && defined(USE_SDL)
                if (!get_check("This would require removing (and dropping) your "
                               "off-hand weapon. Proceed? "))
#else
    ...
    /* Message */
    msg_format("%s %s%s.", act, o_name, item_message_label(slot));
...
void do_cmd_destroy(void)
{
    ...
    o_ptr = &o_list[cave_o_idx[p_ptr->py][p_ptr->px]];
#if defined(__ANDROID__) && defined(USE_SDL)
    /* An inventory action targets the selected item, not the crown underfoot. */
    extern bool android_inventory_has_selection(void);
    if (!android_inventory_has_selection())
#endif
    ...
    if ((o_ptr->tval == TV_STAFF) && (amt < old_number))
