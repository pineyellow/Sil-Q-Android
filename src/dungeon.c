 ...
 */
#if defined(__ANDROID__) && defined(USE_SDL)
static bool android_abandoned_game;
#endif
...
static void process_command(void)
{
    ...
    case KTRL(']'):
    {
        if (get_check("Abandon game and lose your character?"))
        {
            if (!android_save_delete()) {
                msg_print("Could not completely delete the save. Please try again.");
                break;
            }
            android_abandoned_game = TRUE;
            p_ptr->playing = FALSE;
            p_ptr->leaving = TRUE;
        }
        break;
    ...
    case KTRL('\\'):
    {
        extern void android_run_map_tap(void);
        android_run_map_tap();
        break;
    }
#endif
    ...
    {
#if defined(__ANDROID__) && defined(USE_SDL)
        if (!get_check("Save and quit to the main menu?")) break;
        if (!android_save_now()) {
            msg_print(p_ptr->game_type != 0 ? "You cannot save the tutorial."
                : "Saving failed. Your game is still open.");
            break;
        }
        /* Already saved successfully; omit close_game's second save/score UI. */
        android_abandoned_game = TRUE;
#endif
        /* Stop playing */
        p_ptr->playing = FALSE;
...
static void process_player(void)
{
...
#if defined(__ANDROID__) && defined(USE_SDL)
    /* Free commands revisit the input loop without advancing playerturn. */
    bool starting_note_shown = FALSE;
#endif
    ...
    do
    {
...
#if defined(__ANDROID__) && defined(USE_SDL)
        /* Same stable input boundary used by manual saves. Effects and
         * nested selectors have returned; level generation has completed. */
        android_save_checkpoint();
#endif
        ...
        if ((cave_o_idx[p_ptr->py][p_ptr->px] != 0))
        {
            ...
            {
#if defined(__ANDROID__) && defined(USE_SDL)
                if (!starting_note_shown)
                {
                    starting_note_shown = TRUE;
...
#if defined(__ANDROID__) && defined(USE_SDL)
                /* A lifecycle request wakes blocked reads as cancellation.
                 * At the command boundary this is not a player command: go
                 * straight back to the checkpoint without opening the menu,
                 * spending energy, or performing automatic pickup. */
                android_save_poll();
        ...
        /*** Clean up ***/
#if defined(__ANDROID__) && defined(USE_SDL)
        /* Reattach Instant camera only after queued input has been resolved. */
        extern void android_camera_command_finished(void);
        android_camera_command_finished();
#endif
...
void play_game(bool new_game)
{
    ...
    if (new_game)
    {
...
#if defined(__ANDROID__) && defined(USE_SDL)
        {
            ...
            if (android_birth_cancelled)
            {
                android_birth_cancelled = FALSE;
                character_icky--;
                Term_flush();
                return;
            }
...
#if defined(__ANDROID__) && defined(USE_SDL) && defined(SILQ_DEBUG_TUTORIAL_SKILLS)
    /* Runtime-only tutorial test boost; never edit the bundled tutorial save. */
    extern bool android_debug_tutorial_enabled(void);
    if (p_ptr->game_type < 0 && android_debug_tutorial_enabled())
    {
        int skill;
        for (skill = 0; skill < S_MAX; skill++)
            p_ptr->skill_base[skill] = MAX(p_ptr->skill_base[skill], 20);
        if (p_ptr->new_exp < 100000)
        {
            p_ptr->exp += 100000 - p_ptr->new_exp;
            p_ptr->new_exp = 100000;
        }

        /* A loose Silmaril beside the start, using the ordinary floor item path. */
        {
            int y = p_ptr->py, x = p_ptr->px + 1;
            int kind = lookup_kind(TV_LIGHT, SV_LIGHT_SILMARIL);
            if (kind && in_bounds(y, x) && cave_clean_bold(y, x))
            {
                object_type item;
                object_prep(&item, kind);
                floor_carry(y, x, &item);
            }
        }

        /* A genuine three-Silmaril Iron Crown beside the test Silmaril. */
        {
            int y = p_ptr->py, x = p_ptr->px + 2;
            if (in_bounds(y, x) && cave_clean_bold(y, x))
                create_chosen_artefact(ART_MORGOTH_3, y, x, TRUE);
        }

        /* Mixed test kit. Preserve normal tutorial items and pack limits. */
        {
            /* A varied kit for item actions, identification and quantity UI.
             * Keep flavours unknown and leave the tutorial's equipment alone. */
            static const struct { int tval, sval, number, charges; } test_items[] = {
                /* Prioritize melting tests before the pack fills up. */
                { TV_SWORD, SV_MITHRIL_LONG_SWORD, 1, 0 },
                { TV_SHIELD, SV_MITHRIL_SHIELD, 1, 0 },
                { TV_HELM, SV_MITHRIL_HELM, 1, 0 },
                { TV_FOOD, SV_FOOD_RESTORATION, 3, 0 },
                { TV_FOOD, SV_FOOD_VISIONS, 2, 0 },
                { TV_FOOD, SV_FOOD_ENTRANCEMENT, 1, 0 },
                { TV_FOOD, SV_FOOD_RAGE, 7, 0 },
                { TV_POTION, SV_POTION_ESGALDUIN, 5, 0 },
                { TV_POTION, SV_POTION_VOICE, 9, 0 },
                { TV_POTION, SV_POTION_ANTIDOTE, 4, 0 },
                { TV_STAFF, SV_STAFF_UNDERSTANDING, 1, 8 },
                { TV_STAFF, SV_STAFF_RECHARGING, 1, 8 },
                { TV_STAFF, SV_STAFF_SELF_KNOWLEDGE, 1, 2 },
                { TV_STAFF, SV_STAFF_SUMMONING, 1, 4 },
                { TV_HORN, SV_HORN_FORCE, 1, 0 },
                { TV_HORN, SV_HORN_BLASTING, 1, 0 },
                { TV_BOW, SV_SHORT_BOW, 1, 0 },
                { TV_ARROW, SV_NORMAL_ARROW, 19, 0 },
                { TV_LIGHT, SV_LIGHT_LANTERN, 1, 0 },
                { TV_FLASK, 0, 6, 0 },
                { TV_DIGGING, SV_MATTOCK, 1, 0 },
                { TV_RING, SV_RING_COWARDICE, 1, 0 },
                { TV_AMULET, SV_AMULET_LAST_CHANCES, 1, 0 }
            };
            for (int i = 0; i < (int)N_ELEMENTS(test_items); i++)
            {
                object_type item;
                int kind = lookup_kind(test_items[i].tval, test_items[i].sval);
                if (!kind || p_ptr->inven_cnt >= INVEN_PACK) continue;
                object_prep(&item, kind);
                item.number = test_items[i].number;
                if (item.tval == TV_STAFF) item.pval = test_items[i].charges;
                if (item.tval == TV_LIGHT) item.timeout = 1000;
                inven_carry(&item, FALSE);
            }
            p_ptr->update |= (PU_BONUS);
            p_ptr->window |= (PW_INVEN | PW_EQUIP);
        }
...
#if defined(__ANDROID__) && defined(USE_SDL)
    /* Establish the dungeon view before any initial redraw or tutorial note. */
    extern void android_camera_enter_dungeon(void);
    android_camera_enter_dungeon();
#endif
    ...
    /* Android abandonment or successfully saved return to the main menu. */
#if defined(__ANDROID__) && defined(USE_SDL)
    if (android_abandoned_game)
    {
        android_abandoned_game = FALSE;
