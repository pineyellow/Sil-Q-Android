...
static bool load_player_contents(void);
bool load_player(void)
{
    if (!android_save_managed()) return load_player_contents();
    char logical[1024], physical[1024];
    my_strcpy(logical, savefile, sizeof(logical));
    if (!android_save_resolve(physical, sizeof(physical))) {
        msg_print("This save is unavailable or corrupted.");
        message_flush();
        return FALSE;
    }
    my_strcpy(savefile, physical, sizeof(savefile));
    bool ok = load_player_contents();
    my_strcpy(savefile, logical, sizeof(savefile));
    if (ok && !character_loaded) {
        msg_print("This character cannot be resumed.");
        message_flush();
        return FALSE;
    }
    return ok;
