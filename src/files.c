...
void display_player_xtra_info(int mode)
{
    ...
    /* Android allocation screens provide their own footer. */
...
void process_player_name(bool sf)
{
    ...
    {
#if defined(__ANDROID__) && defined(USE_SDL)
        /* Startup and cancelled birth also prepare names. Allocate an Android
         * save identity only when saving a completed character, in save_player. */
#else
        char temp[128];
...
#if defined(__ANDROID__) && defined(USE_SDL)
    if (!character_dungeon) {
        extern char android_birth_random_choice(cptr label, bool* accept_selected);
        bool accept_selected = TRUE;
        for (;;) {
            display_player(0);
            c_put_str(TERM_YELLOW, op_ptr->full_name, 2, 8);
            char choice = android_birth_random_choice("name", &accept_selected);
...
static int final_menu(int* highlight)
{
    ...
    Term_putstr(15, 12, -1, (*highlight == 1) ? TERM_L_BLUE : TERM_WHITE,
        "a) View scores");
#if defined(__ANDROID__) && defined(USE_SDL)
    /* Keep the original action IDs and handlers available for future menus. */
    Term_putstr(15, 13, -1, (*highlight == 8) ? TERM_L_BLUE : TERM_WHITE,
        "b) Exit");
#else
...
#if defined(__ANDROID__) && defined(USE_SDL)
    if (ch == 'a') return (*highlight = 1);
    if (ch == 'b') return (*highlight = 8);
    if (ch == '\r' || ch == '\n' || ch == ' ') return *highlight;
    if (ch == '8' || ch == '2') *highlight = (*highlight == 1) ? 8 : 1;
#else
    if (ch == 'a')
    {
        *highlight = 1;
