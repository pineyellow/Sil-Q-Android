...
/* Keyboard slot suffix for messages; Android keeps the item name alone. */
cptr item_message_label(int i)
{
...
{
#if defined(__ANDROID__) && defined(USE_SDL)
    extern int android_inventory_select(cptr prompt, cptr empty, int mode, int* item);
    int selected = 0;
    int status = android_inventory_select(pmt, str, mode, &selected);
    if (status != -2) {
        /* The bridge validates bounds, location, and filter for pack/floor items. */
        bool allowed = status == 0 && get_item_okay(selected)
            && get_item_allow(selected);
        item_tester_tval = 0;
        item_tester_hook = NULL;
        if (allowed) *cp = selected;
        return allowed;
    }
#endif
    int py = p_ptr->py;
