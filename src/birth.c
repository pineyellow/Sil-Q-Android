...
/* Shared random-choice footer. Navigation never rerolls or enters text. */
char android_birth_random_choice(cptr stage_label, bool* accept_selected)
{
    bool old_hide_cursor = hide_cursor;
    hide_cursor = TRUE;
    for (;;) {
        for (int row = INSTRUCT_ROW; row < Term->hgt; row++)
            Term_erase(0, row, 255);
        int width = 6 + 8 + 6;
        int left = MAX(QUESTION_COL, (Term->wid - width) / 2);
        Term_putstr(left, INSTRUCT_ROW + 1, -1,
            *accept_selected ? TERM_WHITE : TERM_L_BLUE, "Reroll");
        Term_putstr(left + 14, INSTRUCT_ROW + 1, -1,
            *accept_selected ? TERM_L_BLUE : TERM_WHITE,
            format("Accept %s", stage_label));
        Term_fresh();
        char key = inkey();
        if (key == '4' || key == '6') {
...
#if defined(__ANDROID__) && defined(USE_SDL)
        query2 = android_birth_random_choice("history", &accept_selected);
#else
        /* Prompt */
...
#if defined(__ANDROID__) && defined(USE_SDL)
        query2 = android_birth_random_choice("age/height/weight", &accept_selected);
#else
        /* Prompt */
        Term_putstr(QUESTION_COL, INSTRUCT_ROW + 1, -1, TERM_SLATE,
...
static bool player_birth_aux_1(void)
{
    ...
    Term_putstr(QUESTION_COL, INSTRUCT_ROW, -1, TERM_SLATE,
        "Swipe up/down to select; tap to confirm");
    Term_putstr(QUESTION_COL, INSTRUCT_ROW + 1, -1, TERM_SLATE,
        "Swipe left to go back; right to confirm");
#else
    Term_putstr(QUESTION_COL, INSTRUCT_ROW, -1, TERM_SLATE,
    ...
    {
#if defined(__ANDROID__) && defined(USE_SDL)
        if (android_birth_cancelled) return FALSE;
#endif
        clear_question();
...
{
#if defined(__ANDROID__) && defined(USE_SDL)
    extern void android_allocation_input(bool active);
    android_allocation_input(TRUE);
#endif
    int i;
    ...
    while (1)
    {
        ...
        /* Prompt */
#if defined(__ANDROID__) && defined(USE_SDL)
        static const char* attribute_names[A_MAX] = {
            "Strength", "Dexterity", "Constitution", "Grace"
        };
        static const char* attribute_descriptions[A_MAX] = {
            "Increases weapon damage and carrying capacity.",
            "Each point adds +1 to Melee, Archery, Evasion, and Stealth.",
            "Increases maximum health.",
            "Each point adds +1 to Perception, Will, Smithing, and Song."
        };
        /* Clear every description row so shorter attributes leave no suffix. */
        for (int line = INSTRUCT_ROW; line <= INSTRUCT_ROW + 2; line++)
            Term_erase(0, line, 255);
        Term_putstr(QUESTION_COL, INSTRUCT_ROW, -1, TERM_L_BLUE,
            attribute_names[stat]);
        Term_putstr(QUESTION_COL, INSTRUCT_ROW + 1, -1, TERM_WHITE,
            attribute_descriptions[stat]);
        if (stat == A_GRA)
            Term_putstr(QUESTION_COL, INSTRUCT_ROW + 2, -1, TERM_WHITE,
                "Increases maximum Voice.");
#else
        Term_putstr(QUESTION_COL, INSTRUCT_ROW + 1, -1, TERM_SLATE,
        ...
        {
#if defined(__ANDROID__) && defined(USE_SDL)
            android_allocation_input(FALSE);
#endif
            return (FALSE);
        }
...
extern bool gain_skills(void)
{
    ...
    while (1)
    {
...
#if defined(__ANDROID__) && defined(USE_SDL)
        static const char* skill_names[S_MAX] = {
            "Melee", "Archery", "Evasion", "Stealth", "Perception", "Will",
            "Smithing", "Song"
        };
        static const char* skill_descriptions[S_MAX] = {
            "Improves melee accuracy and critical hits.",
            "Improves bow accuracy and critical hits.",
            "Helps you avoid attacks and critical hits.",
            "Makes enemies less likely to notice you.",
            "Helps you discover hidden doors and traps, pick locks, and disarm traps.",
            "Helps you resist fear, confusion, and other harmful effects.",
            "Lets you craft more difficult items at forges.",
            "Increases the strength of your songs."
        };
        /* gain_skills() serves both creation and the in-game character sheet. */
        for (int line = INSTRUCT_ROW; line <= INSTRUCT_ROW + 2; line++)
            Term_erase(0, line, 255);
        Term_putstr(QUESTION_COL, INSTRUCT_ROW, -1, TERM_L_BLUE,
            skill_names[skill]);
        ...
        /* Match the equipment and miscellaneous columns of the skill total. */
        int modifiers[2] = { p_ptr->skill_equip_mod[skill],
            p_ptr->skill_misc_mod[skill] };
        const char* sources[2] = { " from equipment",
            " from background and effects" };
        int modifier_col = QUESTION_COL;
        for (int source = 0; source < 2; source++)
        {
            int value = modifiers[source];
            if (!value) continue;
            const char* separator = modifier_col == QUESTION_COL ? "(" : ", ";
            Term_putstr(modifier_col, INSTRUCT_ROW + 2, -1, TERM_WHITE,
                separator);
            modifier_col += strlen(separator);
            strnfmt(buf, sizeof(buf), "%+d", value);
            Term_putstr(modifier_col, INSTRUCT_ROW + 2, -1,
                value > 0 ? TERM_L_GREEN : TERM_L_RED, buf);
            modifier_col += strlen(buf);
            Term_putstr(modifier_col, INSTRUCT_ROW + 2, -1, TERM_WHITE,
                sources[source]);
            modifier_col += strlen(sources[source]);
        }
        if (modifier_col != QUESTION_COL)
            Term_putstr(modifier_col, INSTRUCT_ROW + 2, -1, TERM_WHITE, ")");
#else
        /* Special Prompt? */
        if (character_dungeon)
...
{
#if defined(__ANDROID__) && defined(USE_SDL)
    extern void android_character_creation(bool active);
    extern void android_character_creation_close(bool active);
    android_character_creation(TRUE);
    android_character_creation_close(TRUE);
    make_random_name(op_ptr->full_name, 14);
#endif
    /* Ask questions */
...
#if defined(__ANDROID__) && defined(USE_SDL)
    android_character_creation(FALSE);
    android_character_creation_close(FALSE);
#endif
