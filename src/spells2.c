...
void self_knowledge(void)
{
    ...
    for (k = 2, j = 0; j < i; j++)
    {
        ...
        if ((k == 21) && (j + 1 < i))
        {
            /* Pause */
#if defined(__ANDROID__) && defined(USE_SDL)
            Term_putstr(1, k + 1, -1, TERM_L_WHITE, "Tap to continue");
            /* Input polling can present a frame before inkey() flushes output.
             * Commit the complete page first, not the stale dungeon cells. */
            Term_fresh();
#else
    ...
    /* Commit the restored dungeon before any subsequent input polling. */
    Term_fresh();
#endif
...
int do_ident_item(int item, object_type* o_ptr)
{
    ...
    {
        msg_format(
            "%^s: %s%s.", describe_use(item), o_name, item_message_label(item));
    }
    else if (item >= 0)
    {
        msg_format("In your pack: %s%s.  %s", o_name, item_message_label(item),
