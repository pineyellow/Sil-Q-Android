...
static void do_cmd_search_skeleton(s16b o_idx)
{
    ...
    else
    {
        if (i_ptr->k_idx)
        {
            ...
            /* Message */
            if (slot >= 0)
                msg_format("You have %s%s.", o_name, item_message_label(slot));
...
void do_cmd_open(void)
{
    ...
    extern int android_tap_direction(void);
    dir = android_tap_direction();
    if (!dir)
#endif
    {
        /* Count closed doors */
...
void do_cmd_exchange(void)
{
    ...
    /* Android's enemy description supplies the chosen adjacent direction. */
#if defined(__ANDROID__) && defined(USE_SDL)
    extern int android_tap_direction(void);
    dir = android_tap_direction();
    if (!dir)
...
void do_cmd_fletchery(void)
{
    ...
    /* Release the Android inventory before the start message and automatic work. */
#if defined(__ANDROID__) && defined(USE_SDL)
    extern void android_inventory_commit(void);
...
void do_cmd_alter(void)
{
    ...
    /* Get a direction */
#if defined(__ANDROID__) && defined(USE_SDL)
    extern bool android_interact_direction(int* direction);
    if (!android_interact_direction(&dir))
        return;
#else
    if (!get_rep_dir(&dir))
        return;
...
#if defined(__ANDROID__) && defined(USE_SDL)
    extern int android_fire_quiver(int quiver);
    android_fire_quiver(quiver);
#endif
#ifdef USE_SDL
    extern void android_fire_aim(bool active, int range);
    android_fire_aim(TRUE, tdis);
    bool aimed = get_aim_dir(&dir, tdis);
    android_fire_aim(FALSE, tdis);
    if (!aimed)
        return;
#else
...
#if defined(__ANDROID__) && defined(USE_SDL)
    /* Aiming can change the pending quiver without restarting targeting. */
    quiver = android_fire_quiver(0);
    item = quiver == 1 ? INVEN_QUIVER1 : INVEN_QUIVER2;
    o_ptr = &inventory[item];
...
void do_cmd_throw(bool automatic)
{
    ...
    {
#if defined(__ANDROID__) && defined(USE_SDL)
        extern bool android_throw_aim(int* direction, int range);
        if (!android_throw_aim(&dir, tdis))
            return;
#else
        if (!get_aim_dir(&dir, tdis))
            return;
