...
void inven_item_describe(int item)
{
    ...
    if (artefact_p(o_ptr) && object_known_p(o_ptr))
    {
        ...
        /* Print a message */
        msg_format(
            "You no longer have the %s%s.", o_name, item_message_label(item));
    }
    else
    {
        /* Get a description */
        ...
        /* Print a message */
        msg_format("You have %s%s.", o_name, item_message_label(item));
    }
}
...
void check_pack_overflow(void)
{
    if (inventory[INVEN_PACK].k_idx)
    {
        ...
        /* Message */
        msg_format("You drop %s%s.", o_name, item_message_label(item));
        ...
        for (j = INVEN_QUIVER1; j <= INVEN_QUIVER2; j++)
        {
            ...
            if (object_similar(j_ptr, o_ptr))
            {
                ...
                else
                {
                    ...
                    /* Message */
                    msg_format("You have %s%s.", j_name, item_message_label(j));
                }
            }
        }
