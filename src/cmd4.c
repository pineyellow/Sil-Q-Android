...
void do_cmd_character_sheet(void)
{
    ...
    extern void android_character_sheet_controls(bool active);
    screen_save();
    android_character_sheet_controls(TRUE);
    /* The skill editor already displays the full character sheet. Enter
     * accepts allocations; Escape restores the original skills and XP. */
    (void)gain_skills();
    screen_load();
    android_character_sheet_controls(FALSE);
#else
    char ch;
...
{
#if defined(__ANDROID__) && defined(USE_SDL)
    extern int android_choose_song(void);
    int song_choice = android_choose_song();
#else
    int i;
    bool done = FALSE;
...
static int artefact_menu_aux(int* highlight)
{
    ...
    num = MAX_CATS + S_MAX;
#if !defined(__ANDROID__) || !defined(USE_SDL)
    // Android retains the default name; custom text entry is not exposed.
    num++;
...
#if defined(__ANDROID__) && defined(USE_SDL)
    /* Reuse the modal [x] and its epoch-validated Escape input. Nested forge
     * menus handle Escape themselves, just as with the keyboard. */
    extern void android_character_sheet_controls(bool active);
    android_character_sheet_controls(TRUE);
#endif
...
#if defined(__ANDROID__) && defined(USE_SDL)
    /* Work and its messages belong to the dungeon, not the design screen. */
    screen_load();
    android_character_sheet_controls(FALSE);
#endif
