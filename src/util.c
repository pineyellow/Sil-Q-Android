...
char inkey(void)
{
    ...
    while (!ch)
    {
        ...
        /* Get a key (see above) */
#if defined(__ANDROID__) && defined(USE_SDL)
        /* Unwind nested creation screens through their normal Escape cleanup. */
        extern bool android_birth_cancelled;
        ch = android_birth_cancelled ? ESCAPE : inkey_aux();
#else
        ch = inkey_aux();
#endif
...
#if defined(__ANDROID__) && defined(USE_SDL)
        /* Android reserves this key for Abandon Game at the command prompt.
         * Keep stripping it from other input, as on the desktop. */
        if (ch == 29 && !inkey_flag)
#else
        if (ch == 29)
#endif
...
static void msg_flush(int x)
{
    byte a = TERM_L_BLUE;
#if defined(__ANDROID__) && defined(USE_SDL)
    extern void android_message_pause(bool active);
    android_message_pause(TRUE);
#endif
        ...
        )
    {
        ...
        while (1)
        {
            ...
            ch = inkey();
#if defined(__ANDROID__) && defined(USE_SDL)
            if (android_save_interrupt()) break;
#endif
            if (quick_messages)
                break;
            if ((ch == ESCAPE) || (ch == ' '))
...
{
#if defined(__ANDROID__) && defined(USE_SDL)
    extern int android_inventory_text(cptr prompt, char* buffer, size_t length);
    int selected = android_inventory_text(prompt, buf, len);
...
{
#if defined(__ANDROID__) && defined(USE_SDL)
    extern int android_inventory_quantity(cptr prompt, int max);
    int selected = android_inventory_quantity(prompt, max);
...
{
#if defined(__ANDROID__) && defined(USE_SDL)
    extern bool android_get_confirmation(cptr prompt);
    return android_get_confirmation(prompt);
#else
    char ch;
